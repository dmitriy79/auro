...
void CNode::PushVersion()
{
    ...
    PushMessage("version", PROTOCOL_VERSION, nLocalServices, nTime, addrYou, addrMe,
                nLocalHostNonce, FormatSubVersion(CLIENT_NAME, CLIENT_VERSION, std::vector<std::string>()), nBestHeight, true);
}
...
#else
    #error "Must have miniupnp version >= 1.6"
#endif
...
bool BindListenPort(const CService &addrBind, std::string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Auroracoin Core is probably already running."), addrBind.ToString());
        else
