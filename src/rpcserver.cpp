                             ...
                             const boost::system::error_code& error)
{
    ...
    AcceptedConnectionImpl<boost::asio::ip::tcp>* tcp_conn = dynamic_cast< AcceptedConnectionImpl<boost::asio::ip::tcp>* >(conn.get());
...
void StopRPCThreads()
{
    ...
    BOOST_FOREACH(const boost::shared_ptr<boost::asio::ip::tcp::acceptor> &acceptor, rpc_acceptors)
    {
        acceptor->cancel(ec);
        if (ec)
    ...
    rpc_acceptors.clear();
    BOOST_FOREACH(const PAIRTYPE(std::string, boost::shared_ptr<boost::asio::deadline_timer>) &timer, deadlineTimers)
    {
